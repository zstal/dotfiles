...
local SIXTEENTH_GRID_SIZE = GRID_SIZE / 16
local COMPACT_SIZE = GRID_SIZE - 2 * SIXTEENTH_GRID_SIZE
...
function This.windowCenter(window)
 if window == nil then
  window = hs.window.focusedWindow()
end
if window then
  toScreen = nil
  inBounds = true
  window:centerOnScreen(toScreen, inBounds)
end
end
